...
export function UserRolesSection({ userRoles, availableRoles, onRemoveRole, onAssignRole, isLoading }: UserRolesSectionProps) {
  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm mt-6">
      <div className="mb-4">
        ...
        <p className="text-sm text-gray-500">Roles determine what this user can access in the admin panel</p>
      </div>
      ...
      <div className="mb-6">
        ...
        {hasNoRoles ? (
          <div className="flex items-center gap-2 rounded-lg bg-yellow-50 border border-yellow-200 p-3">
            ...
            <p className="text-sm text-yellow-700">No roles assigned — this user has no permissions in the admin panel.</p>
          </div>
        ) : (
          <div className="flex flex-wrap gap-2">
            {userRoles.map((role) => (<RoleBadge key={role.id} role={role} onRemove={onRemoveRole} canRemove={true} />))}
          </div>
        )}
      </div>
      ...
      <div>
        ...
        <AssignRoleDropdown availableRoles={availableRoles} onAssign={onAssignRole} isLoading={isLoading} />
      </div>
    </div>
  );
}