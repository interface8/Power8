...
import { System, ControlAction, ControlLog } from "@/types/admin-solar-system";
    ...
    <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm hover:shadow-md transition-shadow relative">
      ...
      <div className="flex flex-col gap-0.5 pr-24">
        ...
        <p className="text-xs text-gray-400 font-mono">
          {system.deviceId || system.id.slice(0, 8)}
        </p>
      </div>

      {/* Action Buttons */}
      <div className="mt-5">
        <div className="grid grid-cols-3 gap-2 bg-white p-0">
          {/* Enable */}
          <button
            onClick={() => onAction(system, "ENABLE")}
            disabled={system.status === "ENABLED" || system.status === "ACTIVE"}
            className={`
              flex h-10 items-center justify-center gap-2 rounded-lg
              border text-sm font-medium transition-all
              ${
                system.status === "ENABLED" || system.status === "ACTIVE"
          ...
          <button
            ...
            disabled={system.status === "LIMITED"}
            className={`
              flex h-10 items-center justify-center gap-2 rounded-lg
              border text-sm font-medium transition-all
              ${
