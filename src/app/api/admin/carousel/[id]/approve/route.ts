...
// PATCH /api/admin/carousel/:id/approve
export async function PATCH(
  ...
  try {
    const data = await carouselService.approveSlide(params.id);
    return jsonResponse({ data });
  } catch (error: unknown) {
    const message =
      error instanceof Error ? error.message : "Failed to approve carousel slide";
